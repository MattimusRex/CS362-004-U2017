...
void great_hallAction(struct gameState*, int, int);
...
void adventurerAction(struct gameState *state, int currentPlayer){
  int cardDrawn, temphand[MAX_HAND], drawntreasure = 0, z = 0;
...
void smithyAction(struct gameState *state, int currentPlayer, int handPos){
  int i, j = 3;
    if(Random() < .04){
        j = 4;
     }
...
void council_roomAction(struct gameState* state, int currentPlayer, int handPos){
    int i;
    for (i = 0; i < 4; i++)
    {
        drawCard(currentPlayer, state);
...
int remodelAction(struct gameState* state, int currentPlayer, int handPos, int choice1, int choice2 ){
    int j, i;
...
void great_hallAction(struct gameState *state, int currentPlayer, int handPos) {
    //+1 Card
    drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  //refactored functions
    if(card == adventurer) {
      ...
      smithyAction(state, currentPlayer, handPos);
        return 0;
    }
    else if(card == council_room){

        council_roomAction(state, currentPlayer, handPos);
        return 0;
    }
    else if(card == remodel){

     if(remodelAction(state, currentPlayer, handPos, choice1 ,choice2) == -1){
        ...
        great_hallAction(state, currentPlayer, handPos);
        return 0;
    }
