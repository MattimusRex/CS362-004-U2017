...
int adventurerEffect(struct gameState *state) {
	...
	int cardDrawn;
	int z = 0;
	//bug: should be while(drawntreasure < 2)
...
int smithyEffect(struct gameState *state, int handPos) {
	...
	//bug: this card should not increase the number of actions
	state->numActions++;
...
int salvagerEffect(struct gameState *state, int choice1, int handPos) {
	int currentPlayer = whoseTurn(state);
	//+1 buy
	state->numBuys++;
	...
	if (choice1)
	{
		...
		//trash card
		//bug: the last argument should be set to 1 to trash the card.  This only discards the card.
...
int council_roomEffect(struct gameState *state, int handPos) {
	int currentPlayer = whoseTurn(state);
	int i;
	//+4 Cards
	for (i = 0; i < 4; i++)
	...
	//bug: last argument should be 0.  This bug trashes the card instead of discards it.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  case council_room:
	  council_roomEffect(state, handPos);
	  return 0;
  ...
  case salvager:
	  salvagerEffect(state, choice1, handPos);
	  return 0;
